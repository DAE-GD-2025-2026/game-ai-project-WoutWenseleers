...
#pragma region UI
	...
	{
		ImGui::TextColored(ImVec4(1,0,0,1), "TrimWorld not assigned!");
	}
	ImGui::Spacing();
...
#pragma region PerAgentUI
	...
	for (int i{ 0 }; i < SteeringAgents.size(); ++i)
	{
		...
		if (ImGui::CollapsingHeader(agentHeader.c_str()))
		{
			...
			if (ImGui::Combo("", &a.SelectedBehavior, "Drunk\0EvadeAgent\0", 1))
			{
...
void ALevel_CombinedSteering::SetAgentBehavior(ImGui_Agent& Agent)
{
	...
	case BehaviorTypes::Drunk:
		Agent.Behavior = std::make_unique<BlendedSteering>(
			std::vector<BlendedSteering::WeightedBehavior>{
				{ new Seek(), 0.5f },
				{ new Wander(), 0.5f}
		});
		break;
	case BehaviorTypes::EvadingAgent:
		Agent.Behavior = std::make_unique<PrioritySteering>(
